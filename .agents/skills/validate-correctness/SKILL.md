...
mkdir -p $REPOROOT/workspace/logging
```
...
```bash
cd $WORKTREE && srun -N <nodes> -W 0 -o workspace/logging/validate-<model>-base.log .agents/skills/validate-correctness/scripts/launch_validate.sh --model <model> --pipeline-parallel-size <PP> --expert-parallel-size <EP>
cd $REPOROOT && srun -N <nodes> -W 0 -o workspace/logging/validate-<model>-feat.log .agents/skills/validate-correctness/scripts/launch_validate.sh --model <model> --pipeline-parallel-size <PP> --expert-parallel-size <EP>
```
...
```bash
python3 .agents/skills/validate-correctness/scripts/compare.py workspace/logging/validate-<model>-base.log workspace/logging/validate-<model>-feat.log
```
