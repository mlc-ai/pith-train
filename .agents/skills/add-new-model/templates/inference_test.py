...
def _load_dcp_canonical(dcp_path: Path) -> dict:
    ...
    prefix = "app.model."
    sd = {}
    # Allocate on CPU: a 20B model is ~42 GB, a 120B model ~230 GB, so a CUDA
    # default device OOMs before the model is even built.
...
def _load_localized_from_canonical(canonical: dict, dualpipev: DualPipeV) -> dict:
    # Remap canonical per-expert indexed keys (...mlp.experts.3.gate_proj.weight) to the
    # runtime GroupedLinear layout (...mlp.experts.gate_proj.weight), gathering this rank's
    # [start, end) EP slice of experts. Mirrors modules.checkpoint.to_localized_model without
    # DTensor wrapping since inference has no FSDP-sharded parameters.
    import re

    from pithtrain.modules.checkpoint import MODULE_PREFIX_RE
    ...
    for canon, tensor in canonical.items():
        ...
        if m:
            ...
            if moe is None or not hasattr(moe, "experts_per_rank"):
                continue
            start = distributed.ep_rank * moe.experts_per_rank
            end = start + moe.experts_per_rank
            idx = int(idx_str)
            if start <= idx < end:
...
def build_dualpipev(config, dtype: torch.dtype) -> DualPipeV:
    # Build the DualPipeV wrapper around this pp rank's two model chunks (phase 0 and
    # phase 1). Inference doesn't wrap with FSDP: DP=CP=1, so sharding has no benefit and
    # DTensor-in-state_dict makes the localization pass messier. DualPipeV and the model
    # read parallelism degrees and process groups off the distributed context.
    #
    # These models build training.Linear / training.GroupedLinear at construction time;
    # bind the BF16 backend (setup_model normally does this).
    training.fp8 = False
    training.Linear = nn.Linear
    training.GroupedLinear = GroupedLinear

    modules = [
        # TODO_MODEL: swap HFPrefixModel for your model class; if it takes extra kwargs, pass them.
        HFPrefixModel(config, phase=0),
        HFPrefixModel(config, phase=1),
    ]
    modules = nn.Sequential(*modules)
    modules.to(device=torch.cuda.current_device(), dtype=dtype)

    return DualPipeV(modules)
...
def generate(
    ...
    # Greedy decode with a static-seq-len buffer. DualPipeV requires
    # num_chunks >= pp_size * 2; we use one prompt per chunk so num_chunks = len(prompts).
    device = torch.cuda.current_device()
    assert len(prompts) >= distributed.pp_size * 2, (
        f"prompts batch={len(prompts)} must be >= pp_size*2={distributed.pp_size * 2}"
    )
...
def main(args):
    ...
    dualpipev = build_dualpipev(config, dtype)
    ...
    if distributed.rank == 0 and missing:
        print(f"[WARN] missing keys (first 10): {list(missing)[:10]}", flush=True)
    if distributed.rank == 0 and unexpected:
    ...
    min_batch = distributed.pp_size * 2
    ...
    if distributed.rank == 0:
        print(
            f"[INFO] Generating with pp={distributed.pp_size}, ep={distributed.ep_size}, "
            f"batch={len(prompts)}",
            flush=True,
        )
    outputs = generate(
...
if __name__ == "__main__":
    ...
    # Populates the `distributed` runtime context (process group + device mesh).
