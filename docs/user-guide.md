...
This guide is for **users training models with PithTrain**. The [README](../README.md) has the minimal commands to get a run going; this guide fills in the details around them: what hardware you need, the models available, how to configure and scale a run, how to read the output, and how to recover when something goes wrong.

If you want to understand *how the framework works internally* (to modify it), see [`architecture.md`](architecture.md) and [`CONTRIBUTING.md`](../CONTRIBUTING.md) instead.

## Requirements

- **GPU:** NVIDIA Hopper (SM90, e.g. H100) or Blackwell (SM100, e.g. B200). Other architectures are not supported.
- **CUDA:** >= 13.0.
- **Python:** >= 3.12, managed with [uv](https://docs.astral.sh/uv/).
- **Multiple GPUs:** the example configs assume one 8-GPU node, and the framework is designed for multi-GPU training. You can lower the parallelism degrees for fewer GPUs, but the smaller meshes are not the tested defaults.
...
| Qwen3.5-35B-A3B | ~35B / ~3B | `qwen3.5-35b-a3b` | `pp=1, ep=8` | 8 (1 node, H200/B200) |
| GPT-OSS-20B | ~21B / ~3.6B | `gpt-oss-20b` | `pp=1, ep=8` | 8 (1 node) |
| GPT-OSS-120B | ~117B / ~5B | `gpt-oss-120b` | `pp=4, ep=8` | 32 (4 nodes) |

Example dirs live under `examples/pretrain_lm/<dir>/`. The default meshes are starting points — see [Scaling](#scaling-a-run) to change them. DeepSeek-V2-Lite and GPT-OSS-20B fit any single 8-GPU node. **Qwen3-30B-A3B and Qwen3.5-35B-A3B at the default `pp=1, ep=8` need a high-memory single node — 8×H200 (141 GB) or 8×B200 (180 GB); they do not fit 8×H100 (80 GB), where you should instead use two nodes with `pp=2, ep=8` (16 GPUs).** **GPT-OSS-120B requires multiple nodes.**

## End-to-end workflow

Datasets and checkpoints are stored in the `workspace` folder by default. Each example dir under `examples/pretrain_lm/<model>/` is self-contained: a `script.py` (the run config) and a `config.json` (the model architecture).

**1. Prepare the dataset**

```bash
bash examples/tokenize_corpus/launch.sh dclm-<model>
```
...
Edit `examples/pretrain_lm/<model>/script.py` to adjust parallelism, batch size, learning rate, and other hyperparameters. The model architecture is defined in the accompanying `config.json`.

**3. Launch training**

```bash
bash examples/pretrain_lm/launch.sh <model>
```
...
```bash
bash examples/convert_checkpoint/launch.sh <model>
```
...
A run is configured by editing `script.py` — there are no command-line flags to memorize; the file *is* the config. The knobs that matter most:
...
| `training.lr` | Peak learning rate. |
| `training.optimizer` / `training.scheduler` | Builder callables (e.g. `make_muon_optimizer` / `make_wsd_scheduler`); the WSD scheduler takes `start_lr`, `warmup_ratio`, `final_lr`. |
| `training.max_steps` | Total optimizer steps. |
| `training.fp8` | `False` (BF16) or `True` (FP8). See [FP8 training](#fp8-training). |
...
**Sizing memory before you launch.** Use the estimator to check a mesh fits before spending GPU time:
...
- **load-balance-loss** — MoE expert balance; **1.0 is perfect balance** (this is the metric with the coefficient divided out, matching Megatron's convention). Much larger than 1.0 means experts are imbalanced.
...
If `logging.wandb` is configured, the same metrics are logged to Weights & Biases.
...
Set `training.fp8 = True` to train in FP8 (128-element block scaling via DeepGEMM; Hopper and Blackwell). This requires the `deep_gemm` package. Leave it `False` for BF16, which has no extra dependency. FP8 reduces memory and can improve throughput; validate loss parity against BF16 for your setup before committing to a long run.
...
**`deep_gemm` import error.** The FP8 path needs DeepGEMM installed. Either install it or set `training.fp8 = False`.

**A run hangs after one rank fails.** PithTrain installs a fail-fast excepthook and an NCCL heartbeat (driven by `distributed.timeout`, default 15 min) so a crashed rank brings the job down instead of leaving peers to hang. On multi-node runs, raise `distributed.timeout` if legitimate collectives are slower than the heartbeat.

**I switched models and tokenization looks wrong.** Re-run the tokenization step — it is per-tokenizer, and each model reads from its own `workspace/datasets/dclm-baseline/toktxt/<model>/` directory.
